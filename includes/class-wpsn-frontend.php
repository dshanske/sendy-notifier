...
class WPSN_Frontend {
	...
	public function render_engagement() {
		...
		if ( get_option( 'wpsn_enable_popup' ) ) {
			...
			if ( $show ) {
				...
				if($pos==='center') echo "<div id='wpsn-ov' style='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:9998;display:none;'></div>"; ?>
				...
				</div><script>jQuery(document).ready(function($){ if(!localStorage.getItem('wpsn_v18_p')){ setTimeout(function(){ $('#wpsn-p, #wpsn-ov').fadeIn(); }, 1000); } $('#wpsn-p-c').click(function(){ $('#wpsn-p, #wpsn-ov').fadeOut(); localStorage.setItem('wpsn_v18_p','1'); }); });</script><?php
			}
		}
		...
		if ( get_option( 'wpsn_enable_topbar' ) ) { ?>
			...
			</div><script>jQuery(document).ready(function($){ if(!localStorage.getItem('wpsn_v18_b')){ $('#wpsn-b').slideDown(); } $('#wpsn-b-c').click(function(){ $('#wpsn-b').slideUp(); localStorage.setItem('wpsn_v18_b','1'); }); });</script><?php
		}
	...
	public function ajax_subscribe() {
		$res = wp_remote_post( rtrim( get_option( 'wpsn_url' ), '/' ) . '/subscribe', array( 'body' => array( 'api_key' => get_option( 'wpsn_api_key' ), 'name' => $_POST['name'], 'email' => $_POST['email'], 'list' => get_option( 'wpsn_list_id' ), 'boolean' => 'true' ) ) );
		$body = trim( wp_remote_retrieve_body( $res ) );
		if ( $body == '1' || $body == 'true' ) { wp_send_json_success( get_option( 'wpsn_success_message', 'Success!' ) . " Please check your inbox to confirm." ); } else { wp_send_json_error( $body ); }
	}
