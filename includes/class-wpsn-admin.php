...
class WPSN_Admin {
	...
	public function init_settings() {
		...
		foreach ( $options as $opt ) {
			register_setting( 'wpsn-settings-group', $opt ); }
	}
	...
	public function save_metabox_data( $post_id ) {
		if ( ! isset( $_POST['wpsn_metabox_nonce'] ) || ! wp_verify_nonce( $_POST['wpsn_metabox_nonce'], 'wpsn_metabox_action' ) ) {
			return;
		}
					...
					<form method="post" action="options.php">
							...
							<tr id="sch_row" style="<?php echo ( in_array( get_option( 'wpsn_frequency' ), array( 'daily', 'weekly' ) ) ) ? '' : 'display:none;'; ?>">
								<th>Schedule</th><td>
									...
									<span id="sch_day" style="<?php echo ( get_option( 'wpsn_frequency' ) === 'weekly' ) ? '' : 'display:none;'; ?>">
										on <select name="wpsn_send_day">
										<?php
										foreach ( array( 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday' ) as $d ) {
											echo '<option value="' . $d . '" ' . selected( get_option( 'wpsn_send_day' ), $d, false ) . '>' . $d . '</option>';}
										?>
										</select>
									</span>
								</td>
						...
						<table class="form-table">
							...
							<tr><th>Data Elements</th><td>
								<?php
								$en = (array) get_option( 'wpsn_post_elements', array( 'title', 'excerpt' ) );
								foreach ( array(
									'title'      => 'Title',
									'date'       => 'Date',
									'author'     => 'Author',
	...
	public function ajax_fetch_brands() {
		check_ajax_referer( 'wpsn_admin_nonce', 'nonce' );
		$res = wp_remote_post( rtrim( $_POST['url'], '/' ) . '/api/brands/get-brands.php', array( 'body' => array( 'api_key' => $_POST['key'] ) ) );
		wp_send_json_success( json_decode( wp_remote_retrieve_body( $res ), true ) ); }
	public function ajax_fetch_lists() {
		check_ajax_referer( 'wpsn_admin_nonce', 'nonce' );
		$res = wp_remote_post(
			rtrim( $_POST['url'], '/' ) . '/api/lists/get-lists.php',
			array(
				'body' => array(
					'api_key'  => $_POST['key'],
	...
	public function ajax_manual_trigger() {
		check_ajax_referer( 'wpsn_test_nonce', 'nonce' );
		$core = new WPSN_Core();
		$res  = $core->transmit_payload( array( (int) $_POST['post_id'] ) );
		if ( $res === '1' ) {
