			...
			switch ( $key ) {
				...
				case 'excerpt':
					if ( has_excerpt( $post_id ) ) {
						...
						$excerpt = trim( get_the_content( '', false, $post ) );
					...
					$html .= "<p style='color:#444; font-size:14px;'>" . wp_kses_post( $excerpt ) . '</p>';
					break;
				case 'content':
					$html .= '<div>' . wp_kses_post( $post->post_content ) . '</div>';
					break;
				case 'date':
					$html .= "<p style='color:#999; font-size:12px;'>Published: " . get_the_date( '', $post_id ) . '</p>';
					break;
				case 'categories':
					$html .= "<p style='font-size:11px;'>Categories: " . get_the_category_list( ',', '', $post_id ) . '</p>';
					break;
				case 'tags':
					$tags = get_the_tag_list( '', '', '', $post_id );
					if ( $tags ) {
						$html .= "<p style='font-size:11px;'>Tags: $tags</p>";
					} break;
			}
		}
		return $html . '</div>';
